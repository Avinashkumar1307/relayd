...
| R12 | H | 6 | Every transient campaign state (`validating`, `queueing`, `pausing`, `cancelling`) has a deadline; `campaign-reconcile` force-exits it after 10 min (`pausing → paused`, `cancelling → cancelled`, `validating/queueing → failed with reason`). | Put a campaign in `pausing` with one stuck recipient; run reconciler; assert `paused`. || `packages/campaigns/test/sweeper.test.ts` (all four transient states have exits, the ten-minute deadline, the guarded force, no exit into another transient state). |
